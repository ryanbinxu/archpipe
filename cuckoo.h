...
cuckoo* cuckoo_init(uint32_t init_cap);

// insert value into cuckoo table
// return 1 if key already exists
uint8_t cuckoo_insert(cuckoo *cuck, char *key, void *val);

// return true if key exists
uint8_t cuckoo_exists(cuckoo *cuck, char *key);

// get value from cuckoo table
void* cuckoo_get(cuckoo *cuck, char *key);

// remove value from cuckoo table
// return void * to data
// caller responsible for freeing data
void* cuckoo_remove(cuckoo *cuck, char *key);

// destroy cuckoo table
// deep == 1 will free values
void cuckoo_destroy(cuckoo *cuck, uint8_t deep);
#endif
