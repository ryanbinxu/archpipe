...
void display_stripe_cons(int** stripe_cons){
	...
	for(int i = 0;i<stripe_num;i++){
		for (int j = 0; j < erasure_k; ++j){
			...
			fprintf(stripes,"%d-%d,%d  ",blockid,placement[blockid*rep_num]/(node_num/zone_num),placement[blockid*rep_num+1]/(node_num/zone_num));
		}
		fprintf(stripes,"\n");
	}
	fclose(stripes);
...
int main(int argc, char const *argv[])
{
	...
	for (int i = 0; i < stripe_num; ++i){
		...
		if(Archival_Type==0||Archival_Type==1||Archival_Type==2){
